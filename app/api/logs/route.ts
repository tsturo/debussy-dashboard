...
import { LogEntry, LogsResponse, AgentType, LogFilters } from '@/lib/types';
...
async function getAgentLogs(agent: AgentType): Promise<LogEntry[]> {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const agentsToQuery = agent ? [agent as AgentType] : VALID_AGENTS;

    let allLogs: LogEntry[] = [];
    for (const agentType of agentsToQuery) {
      const logs = await getAgentLogs(agentType);
      allLogs.push(...logs);
    }
    ...
    const agents = Array.from(new Set(allLogs.map(log => log.agent))) as AgentType[];
